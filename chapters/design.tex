...
\paragraph{} Let us define $\mathcal{R}$, the set of all \textit{reservations}, as follows.
...
\paragraph{} We introduce, for a process $A$, additional state; $A_{r} \subseteq \mathcal{R}$, the set of all reservations is holds. Once a decision has been made it is important for a reference monitor to be able to change it, revoking access if required. Thus we further define a notion of \textit{validity} indicator function, $\mathcal{V}: \mathcal{R} \mapsto \{0,1\}$. A reservation can only be used to obtain access to a resource if it is valid; invalid reservations are deleted.
